...
class TestPath < MiniTest::Unit::TestCase
	...
	def test_build_path_4
		...
		assert is_hamming_distance_1?(path)
		assert_equal(path.nodes.first.word, 'fike')
	...
	def test_different_length
		assert_raises ArgumentError do 
			Path.new(__dir__ + '/fixtures/words_short', 'short', 'looooonnnggg')
		end
	end

	def test_non_existing_words
		assert_raises ArgumentError do 
			Path.new(__dir__ + '/fixtures/words_short', 'cualquiera1', 'cualquiera2')
		end
	end
